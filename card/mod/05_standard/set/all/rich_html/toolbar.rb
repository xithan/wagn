...
format :html do
  ...
  view :toolbar do |args|
    collapsed_content = close_link(args.merge(class: 'pull-right visible-xs'))
    navbar("toolbar-#{card.cardname.safe_key}-#{args[:home_view]}",
           toggle_align: :left, class: 'slotter toolbar',
           navbar_type: 'inverse',
           collapsed_content: collapsed_content) do
      [
  ...
  TOOLBAR_VIEW_TITLE =
    { edit: 'content', edit_name: 'name', edit_type: 'type',
      edit_structure: 'structure', edit_nests: 'nests',
      history: 'history', common_rules: 'common',
      recent_rules: 'recent', grouped_rules: 'all', edit_nest_rules: 'nests'
    }.freeze
  def toolbar_view_title view
    if view == :edit_rules
      current_set_card.name
    else
      TOOLBAR_VIEW_TITLE[view]
    end
  end
    ...
    toolbar_split_button 'edit', { view: :edit }, args do
      {
        ...
        edit_nests: (_render_edit_nests_link if !card.structure &&
                                                args[:nested_fields].present?),
        structure:  (smart_link 'structure',
                     view: :edit_structure if structure_editable?),
  ...
  def toolbar_split_button name, button_args, args
    button = button_link name, button_args,
  ...
  def toolbar_pin_button
    active_class = toolbar_pinned? ? 'active' : 'inactive'
    button_tag glyphicon('pushpin'),
               situation: :primary, remote: true,
               title: "#{'un' if toolbar_pinned?}pin",
               class: "btn-toolbar-control toolbar-pin #{active_class}"
  end

  view :toolbar_buttons do |args|
    show_delete_button = card.ok?(:delete) ? :show : :hide
    wrap_with(:div, class: 'btn-group') do
      [
        _optional_render(:delete_button,  args, show_delete_button),
